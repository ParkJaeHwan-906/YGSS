...
def find_best_regression_columns(merged_data: pd.DataFrame = None):
    ...
    best_combo = min(results, key=lambda combo: results[combo]['MSE'])
    
    print("\n최적의 회귀 특성 조합:", best_combo)
    print("최고 MAE:", results[best_combo]['MSE'])
    
    return list(best_combo)

def main(asset_data: pd.DataFrame = None, asset_type: str = None, sequence_length: int = 12, upload_to_fastapi: bool = True, **legacy_kwargs):
    configure_gpu()
    if asset_data is None and isinstance(legacy_kwargs, dict) and 'etf_data' in legacy_kwargs:
        asset_data = legacy_kwargs['etf_data']
    ...
    local_model_path = f"{juptyer_base_path}/saved_models"
    model_manager = ModelManager(base_path=local_model_path, asset_type=asset_type)
    try:
        ...
        # X_train 스케일링 (수정된 부분)
        # X_train_unscaled는 (samples, seq_len, num_features) 형태
        # num_features는 regression_group의 길이와 같아야 합니다.
        num_features = len(regression_group)
        
        # X_train_unscaled의 형태를 (samples * seq_len, num_features)로 변경
        X_train_reshaped = X_train_unscaled.reshape(-1, X_train_unscaled.shape[-1])
        
        # data_for_lstm의 컬럼 순서와 일치하는지 확인해야 합니다.
        # lstm_cols = regression_group + ['id', 'return'] 이므로
        # 앞의 regression_group 부분만 슬라이싱하여 스케일링합니다.
        X_train_features_only = X_train_unscaled[:, :, :num_features]
        X_test_features_only = X_test_unscaled[:, :, :num_features]
        
        # 피처만 포함된 데이터를 reshape하여 스케일링
        X_train_reshaped = X_train_features_only.reshape(-1, num_features)
        X_train_scaled_reshaped = feature_scaler.fit_transform(X_train_reshaped)
        X_train = X_train_scaled_reshaped.reshape(X_train_unscaled.shape[0], X_train_unscaled.shape[1], num_features)
        
        X_test_reshaped = X_test_features_only.reshape(-1, num_features)
        X_test_scaled_reshaped = feature_scaler.transform(X_test_reshaped)
        X_test = X_test_scaled_reshaped.reshape(X_test_unscaled.shape[0], X_test_unscaled.shape[1], num_features)
    ...
    if upload_to_fastapi and saved_hash:
        try:
            ...
            if not metadata_path.exists():
                ...
                upload_url = f"{fastapi_base_path}/models/upload"
...
if __name__ == "__main__":
    ...
    print("ETF 파이프라인 실행")
    print("="*60)
    etf_results = main(etf_data, "etf")
    print("\n" + "="*60)
    print("Fund 파이프라인 실행")
    print("="*60)
    fund_results = main(fund_data, "fund")
